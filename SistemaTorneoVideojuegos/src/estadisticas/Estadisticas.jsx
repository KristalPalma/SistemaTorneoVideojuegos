...
import { comparisonRows, loadStatistics, loadDashboardStatistics } from './estadisticasService.js'
...
export function StatisticsResults({ metrics, games, gameId, dashboard = false }) {
  const rows = comparisonRows(games, metrics, gameId)
  const highest = rows.reduce((max, row) => Math.max(max, row.average ?? 0), 0)
  const cards = [
    ['players', dashboard && gameId === undefined ? 'Total de jugadores' : 'Jugadores participantes', dashboard ? metrics.players : metrics.participants, 'purple'],
    ['games', 'Videojuegos', dashboard ? metrics.games : rows.length, 'blue'],
  ...
  return <>
    <div className="statistics-grid">{cards.filter(([icon]) => (gameId === undefined || icon !== 'games') && (!dashboard || !['max', 'min'].includes(icon))).map(([icon, label, value, color]) => <article className="statistics-card" key={label}>
    ...
    {!dashboard && <section className="statistics-chart" aria-labelledby="chart-title">
...
export default function Estadisticas({ dashboard = false }) {
  ...
  useEffect(() => {
    const controller = new AbortController()
    const load = dashboard ? loadDashboardStatistics : loadStatistics
    load(gameId, controller.signal).then(metrics => {
      if (!controller.signal.aborted) setState({ metrics })
    }).catch(() => { if (!controller.signal.aborted) setState({ error: true }) })
    return () => controller.abort()
  }, [gameId, retry, dashboard])
  ...
  return <section className="module-panel statistics-page" aria-labelledby="statistics-title">
    ...
    {dashboard ? <h2 id="statistics-title">Resumen del torneo</h2> : <h1 id="statistics-title">Estadísticas del torneo</h1>}
    ...
    <div className="statistics-filter">
      <div><label htmlFor="statistics-game">Videojuego</label>
        <BuscarOpcion key={filterReset} id="statistics-game" items={games.data} value={selected}
          required={false} disabled={games.loading || games.error} onChange={change}
          placeholder="Escribe para buscar un videojuego…" />
      </div>
      <button className="login-button" onClick={() => { change(''); setFilterReset(value => value + 1) }}>Restablecer filtro</button>
    </div>
    {state.error || games.error ? <div className="auth-error" role="alert"><p>No fue posible cargar las estadísticas. Comprueba la conexión e intenta nuevamente.</p><button className="login-button" onClick={reload}>Reintentar</button></div>
      : state.loading || games.loading ? <p className="statistics-loading" role="status">Cargando estadísticas…</p>
        : <StatisticsResults metrics={state.metrics} games={games.data} gameId={gameId} dashboard={dashboard} />}
  </section>
}
